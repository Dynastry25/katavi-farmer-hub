...
function App() {
  ...
  const handleAddCrop = async (cropData) => {
    try {
      const res = await cropsAPI.create(cropData);
  ...
  // Update crop
  const handleUpdateCrop = async (cropId, updatedData) => {
    try {
      const res = await cropsAPI.update(cropId, updatedData);
      setCrops(prev => prev.map(crop => crop._id === cropId ? res.data : crop));
    } catch (error) {
