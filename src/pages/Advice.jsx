...
const Advice = ({ onPageChange, onAuth, user }) => {
    ...
    const fetchData = async () => {
      try {
        const [articlesRes, expertsRes] = await Promise.all([
          ...
          adviceAPI.getExperts(),
        ]);
        if (articlesRes.data && articlesRes.data.length > 0) setApiArticles(articlesRes.data);
        if (expertsRes.data && expertsRes.data.length > 0) setApiExperts(expertsRes.data);
      } catch (err) {
  ...
  const articles = apiArticles.length > 0 ? apiArticles.map(a => ({ ...a, id: a._id })) : [
    {
      id: 1,
  ...
  const experts = apiExperts.length > 0 ? apiExperts.map(e => ({ ...e, id: e._id })) : [
    {
      id: 1,
      name: 'Dk. Anna Mrosso',
