...
const Suppliers = ({ onPageChange, onAuth, user }) => {
  ...
  useEffect(() => {
    const fetchSuppliers = async () => {
      try {
        ...
        if (res.data && res.data.length > 0) setApiSuppliers(res.data);
      } catch (err) {
  ...
  const suppliers = apiSuppliers.length > 0 ? apiSuppliers : [
    { id: 1, name: 'Agro Supplies Ltd', category: 'fertilizers', location: 'Mpanda Mjini', rating: 4.7, products: ['CAN', 'UREA', 'NPK'], contact: '+255 789 123 456', email: 'info@agrosupplies.co.tz', description: 'Wauzaji wakuu wa mbolea za kisasa kwa wakulima', delivery: true, verified: true },
    { id: 2, name: 'SeedCo Tanzania', category: 'seeds', location: 'Mlele', rating: 4.8, products: ['Mahindi DK90', 'Mpunga SARO', 'Maharage'], contact: '+255 789 654 321', email: 'sales@seedco.tz', description: 'Mbegu bora za kilimo zenye uhakika wa mazao', delivery: true, verified: true },
    { id: 3, name: 'Farm Tools Tanzania', category: 'tools', location: 'Nsimbo', rating: 4.6, products: ['Jembe', 'Panga', 'Teko'], contact: '+255 789 321 654', email: 'tools@farmtools.tz', description: 'Vifaa vyote vya kilimo kwa bei nafuu', delivery: true, verified: true },
  ];
