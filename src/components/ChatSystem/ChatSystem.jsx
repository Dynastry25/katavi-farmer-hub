...
const ChatSystem = ({ user, onClose, onPageChange }) => {
  ...
  useEffect(() => {
    const fetchConversations = async () => {
      try {
        ...
        if (res.data && res.data.length > 0) setApiConversations(res.data);
      } catch (err) {
  ...
  // Sample chat data
  const chats = apiConversations.length > 0 ? apiConversations.map(c => ({ ...c, id: c._id })) : [
    {
      id: 1,
      name: 'Juma Mwinyi',
      type: 'farmer',
  ...
  // Sample messages
  const sampleMessages = Object.keys(apiMessages).length > 0 ? apiMessages : {
    1: [
  ...
  useEffect(() => {
    if (activeChat) {
      const fetchMessages = async () => {
        try {
          const res = await chatAPI.getMessages(activeChat._id || activeChat.id);
          if (res.data && res.data.length > 0) {
            setApiMessages(prev => ({ ...prev, [activeChat._id || activeChat.id]: res.data.map(m => ({ ...m, id: m._id })) }));
          }
        } catch (err) {
      ...
      try {
        if (activeChat) {
          await chatAPI.sendMessage({
            conversation: activeChat._id || activeChat.id,
            text: newMessage,
          });
        }
      } catch (err) {
